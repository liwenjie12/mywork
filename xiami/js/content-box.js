let contentBox=new Vue({
    ...
    components:{
        contentList:{
            ...
            template:"<div :style='divStyle'><ul :style='ulStyle'><li v-for='item in book' :style='liStyle'><h4 :style='first'><a>{{item.header}}</a></h4><p :style='twice'><a :style='twicea'>{{item.content}}</a></p><p :style='thirth'><a>{{item.price}}</a></p><img :data-original='item.path' :style='imgStyle' class='lazy'/></li></ul><a href='javascript:' :style='leftBtn' @click='move($event,\"add\")' v-show='switched'><</a><a href='javascript:' :style='rightBtn' @click='move($event,\"minus\")' v-if='switched'>></a></div>"
        }
    }
})